...
interface IAbbreviator {
  (number: number | string): string | number;
}

const abbreviator: IAbbreviator = (amount) => {
  const validatedAmount = validateInput(amount);
  const isNegative = validatedAmount < 0;
  const NUM = Math.abs(validatedAmount as number);
  const isThousand = () => {
    if (NUM >= THOUSAND) {
...
interface IEvaluator {
  (number: number, base: number, sign: string): string;
}
const evaluator: IEvaluator = (number, base, sign) => {
...
const validateInput: IValidator = (input) => {
  let output = input;
